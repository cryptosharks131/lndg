                    ...
                    if target_fee_rate > 0:
                        ...
                        if target_value >= value_per_fee:
                            ...
                            if Rebalancer.objects.filter(last_hop_pubkey=inbound_pubkey.remote_pubkey).exclude(status=0).exists():
                                ...
                                if last_rebalance.value != target_value or last_rebalance.status in [2, 6] or (last_rebalance.status in [1, 3, 4, 5, 7, 400] and (int((datetime.now() - last_rebalance.stop).total_seconds() / 60) > 30)):
