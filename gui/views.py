...
def home(request):
    if request.method == 'GET':
        ...
        rebalances = Rebalancer.objects.all().annotate(ppm=(Sum('fee_limit')/Sum('value'))*1000000).order_by('-id')
...
def channel(request):
    if request.method == 'GET':
        ...
        if Channels.objects.filter(chan_id=chan_id).exists():
            ...
            rebalancer_df = DataFrame.from_records(Rebalancer.objects.filter(last_hop_pubkey=channels_df['remote_pubkey'][0]).annotate(ppm=(Sum('fee_limit')/Sum('value'))*1000000).order_by('-id').values())
...
def rebalancing(request):
    if request.method == 'GET':
        ...
        context = {
            'channels': channels_df.to_dict(orient='records'),
            'rebalancer': Rebalancer.objects.all().order_by('-id').annotate(ppm=(Sum('fee_limit')/Sum('value'))*1000000)[:20],
