...
def channels(request):
    if request.method == 'GET':
        ...
        for channel in channels:
            ...
            detailed_channel['costs_7day'] = 0 if detailed_channel['rebal_out_7day'] == 0 else int(payments.filter(creation_date__gte=filter_7day).filter(payment_hash_in=invoices.filter(settle_date__gte=filter_7day).filter(chan_in=channel.chan_id).values_list('r_hash')).aggregate(Sum('fee'))['fee__sum'])
            detailed_channel['costs_30day'] = 0 if detailed_channel['rebal_out_30day'] == 0 else int(payments.filter(creation_date__gte=filter_30day).filter(payment_hash_in=invoices.filter(settle_date__gte=filter_30day).filter(chan_in=channel.chan_id).values_list('r_hash')).aggregate(Sum('fee'))['fee__sum'])
