...
def channels(request):
    if request.method == 'GET':
        ...
        for channel in channels:
            ...
            detailed_channel['amt_routed_in_7day'] = 0 if detailed_channel['routed_in_7day'] == 0 else int(forwards.filter(forward_date__gte=filter_7day).filter(chan_id_in=channel.chan_id).aggregate(Sum('amt_in_msat'))['amt_in_msat__sum']/100000000)/10
            ...
            detailed_channel['costs_7day'] = 0 if invoices.filter(settle_date__gte=filter_7day).filter(chan_in=channel.chan_id).count() == 0 else int(payments.filter(creation_date__gte=filter_7day).filter(payment_hash__in=invoices.filter(settle_date__gte=filter_7day).filter(chan_in=channel.chan_id).values_list('r_hash')).aggregate(Sum('fee'))['fee__sum'])
            detailed_channel['costs_30day'] = 0 if invoices.filter(settle_date__gte=filter_7day).filter(chan_in=channel.chan_id).count() == 0 else int(payments.filter(creation_date__gte=filter_30day).filter(payment_hash__in=invoices.filter(settle_date__gte=filter_30day).filter(chan_in=channel.chan_id).values_list('r_hash')).aggregate(Sum('fee'))['fee__sum'])
