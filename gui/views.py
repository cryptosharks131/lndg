...
def suggested_fees(request):
    if request.method == 'GET':
        ...
        if channels_df.shape[0] > 0:
            ...
            channels_df['out_rate'] = channels_df.apply(lambda row: round((row['revenue_7day']/row['amt_routed_out_7day'])*1000000, 0) if row['amt_routed_out_7day'] > 0 else 0, axis=1)
            ...
            invoices_df_7d_sum = DataFrame() if invoices_df_7d.empty else invoices_df_7d.groupby('chan_in', as_index=True).sum()
            invoice_hashes_7d = DataFrame() if invoices_df_7d.empty else invoices_df_7d.groupby('chan_in', as_index=True)['r_hash'].apply(list)
            channels_df['amt_rebal_in_7day'] = channels_df.apply(lambda row: int(invoices_df_7d_sum.loc[row.chan_id].amt_paid/100000)/10 if invoices_df_7d_sum.empty == False and (invoices_df_7d_sum.index == row.chan_id).any() else 0, axis=1)
            ...
            channels_df['rebal_ppm'] = channels_df.apply(lambda row: round((row['costs_7day']/row['amt_rebal_in_7day'])*1000000, 0) if row['amt_rebal_in_7day'] > 0 else 0, axis=1)
            channels_df['new_rate'] = channels_df.apply(lambda row: round((row['out_rate']+row['net_routed_7day']*row['local_fee_rate']*0.05 if row['net_routed_7day'] < 0 else (row['rebal_ppm']+row['net_routed_7day']*row['local_fee_rate']*0.05 if row['rebal_ppm'] > 0 else (row['out_rate']+row['net_routed_7day']*row['local_fee_rate']*0.05 if row['out_rate'] > 0 else row['local_fee_rate']+row['net_routed_7day']*row['local_fee_rate']*0.05))), 0), axis=1)
