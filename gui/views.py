...
def channel(request):
    if request.method == 'GET':
        ...
        if Channels.objects.filter(chan_id=chan_id).exists():
            ...
            channels_df['out_rate'] = int((channels_df['revenue_7day_fees'].iloc[0] / channels_df['amt_routed_out_7day_fees'].iloc[0]) * 1000000) if channels_df['amt_routed_out_7day_fees'].iloc[0] > 0 else 0
            channels_df['rebal_ppm'] = int((channels_df['costs_7day'].iloc[0] / channels_df['amt_rebal_in_7day'].iloc[0]) * 1000000) if channels_df['amt_rebal_in_7day'].iloc[0] > 0 else 0
            channels_df['assisted_ratio'] = round((channels_df['revenue_assist_7day_fees'].iloc[0] if channels_df['revenue_7day_fees'].iloc[0] == 0 else channels_df['revenue_assist_7day_fees'].iloc[0] / channels_df['revenue_7day_fees'].iloc[0]), 2)
