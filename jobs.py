...
def auto_fees(stub):
    ...
    if enabled == 1:
        ...
        if channels_df.shape[0] > 0:
            ...
            channels_df['max_suggestion'] = channels_df.apply(lambda row: int((row['out_rate']+25 if row['out_rate'] > 0 else row['local_fee_rate'])*1.15) if row['in_percent'] > 25 else int(row['local_fee_rate']), axis=1)
            ...
            channels_df['min_suggestion'] = channels_df.apply(lambda row: int((row['out_rate']-50 if row['out_rate'] > 0 else row['local_fee_rate'])*0.75) if row['out_percent'] > 25 else int(row['local_fee_rate']), axis=1)
